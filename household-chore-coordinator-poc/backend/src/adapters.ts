...
export class Infrastructure {
  ...
  async connectPostgres(): Promise<void> {
    const databaseUrl = process.env.CHORE_DATABASE_URL || "memory://";
    if (databaseUrl === "memory://") return;
    try {
      this.pgPool = new pg.Pool({ connectionString: databaseUrl });
      await this.pgPool.query(`CREATE TABLE IF NOT EXISTS chore_snapshots (id TEXT PRIMARY KEY, state JSONB NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
      await this.pgPool.query(`CREATE TABLE IF NOT EXISTS chore_completion_events (event_key TEXT PRIMARY KEY, topic TEXT NOT NULL, payload JSONB NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
  ...
  async connectRedis(): Promise<void> {
    const redisUrl = process.env.CHORE_REDIS_URL || "memory://";
  ...
  async connectKafka(): Promise<void> {
    const brokers = (process.env.CHORE_KAFKA_BROKERS || "memory://").split(",").filter(Boolean);
    if (brokers.includes("memory://")) return;
    try {
      const kafka = new Kafka({ clientId: "household-chore-coordinator", brokers });
      this.producer = kafka.producer();
      this.consumer = kafka.consumer({ groupId: process.env.CHORE_KAFKA_GROUP || "household-chore-coordinator-poc" });
  ...
  async save(state: Record<string, unknown>): Promise<void> {
    ...
    if (this.pgPool) {
      await this.pgPool.query(`INSERT INTO chore_snapshots (id, state, updated_at) VALUES ($1, $2::jsonb, NOW()) ON CONFLICT (id) DO UPDATE SET state = EXCLUDED.state, updated_at = NOW()`, ["demo", JSON.stringify(state)]);
    }
    if (this.redis) await this.redis.setEx("chores:snapshot", 300, JSON.stringify(state));
  }

  async load(): Promise<Record<string, unknown> | null> {
    if (this.pgPool) {
      const result = await this.pgPool.query("SELECT state FROM chore_snapshots WHERE id = $1", ["demo"]);
      return result.rows[0]?.state || null;
    }
    return this.snapshot;
  }

  async appendCompletion(topic: string, eventKey: string, payload: Record<string, unknown>): Promise<void> {
    if (!this.pgPool) return;
    await this.pgPool.query(`INSERT INTO chore_completion_events (event_key, topic, payload) VALUES ($1, $2, $3::jsonb) ON CONFLICT (event_key) DO NOTHING`, [eventKey, topic, JSON.stringify(payload)]);
  }

  async mirrorProjections(tasks: unknown, reminders: unknown, jobs: unknown): Promise<void> {
    if (!this.redis) return;
    await this.redis.set("chores:tasks", JSON.stringify(tasks));
    await this.redis.set("chores:reminders", JSON.stringify(reminders));
    await this.redis.set("chores:jobs", JSON.stringify(jobs));
  }
