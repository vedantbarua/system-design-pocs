...
import { createSeededCoordinator, routeEventKey, type RouteEvent } from "./core.js";

const PORT = Number(process.env.PORT || 8196);
const HOST = process.env.HOST || "127.0.0.1";
const TOPIC = process.env.WASTE_ROUTE_TOPIC || "home.waste.route-events";
...
async function persist(event?: RouteEvent) {
...
async function processRouteEvent(message: Record<string, unknown>) {
  const result = coordinator.ingest({ ...message, source: "kafka" } as never);
  await persist(result.event);
}

await infra.consume(TOPIC, processRouteEvent);
...
app.post(
  "/api/events",
  route(async (req, res) => {
    const input = { ...req.body, delayMinutes: req.body.delayMinutes === undefined ? undefined : Number(req.body.delayMinutes), source: "api" };
    await infra.publish(TOPIC, input.routeId, input);
...
app.post(
  "/api/events/publish",
  route(async (req, res) => {
    coordinator.findSchedule(req.body.scheduleId);
    coordinator.findRoute(req.body.routeId);
    routeEventKey(req.body);
    await infra.publish(TOPIC, req.body.routeId, req.body);
...
app.post(
  ...
  route(async (_req, res) => {
    const result = await infra.drain(TOPIC, processRouteEvent);
    await persist();
    res.json(result);
  })
);
...
const server = app.listen(PORT, HOST, () => {
  console.log(`Waste pickup API listening on http://${HOST}:${PORT} (${infra.mode})`);
});
