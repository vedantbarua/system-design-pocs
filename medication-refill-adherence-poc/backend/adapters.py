"""Optional PostgreSQL and Redis adapters for the medication POC."""
...
class Infrastructure:
    ...
    def connect(self) -> "Infrastructure":
        database_url = os.getenv("MEDICATION_DATABASE_URL", "memory://")
        redis_url = os.getenv("MEDICATION_REDIS_URL", "memory://")
        ...
        try:
            ...
            self.postgres.execute(
                """
                CREATE TABLE IF NOT EXISTS medication_poc_snapshots (
                    id TEXT PRIMARY KEY,
    ...
    def save(self, state: dict[str, Any]) -> None:
        ...
        self.postgres.execute(
            """
            INSERT INTO medication_poc_snapshots (id, state, updated_at)
            VALUES (%s, %s::jsonb, NOW())
            ON CONFLICT (id) DO UPDATE SET state = EXCLUDED.state, updated_at = NOW()
            """,
            ("demo", payload),
        )
        self.redis.setex("medication-poc:snapshot", 300, payload)

    def mirror_jobs(self, jobs: list[dict[str, Any]]) -> None:
        if self.mode == "memory":
            return
        key = "medication-poc:notification-jobs"
