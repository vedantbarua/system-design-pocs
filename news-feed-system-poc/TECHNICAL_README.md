# Technical README: News Feed System POC

This document explains the architecture, flow, and file-by-file purpose of the news feed proof-of-concept.
...
- **Service**: `NewsFeedService` manages user creation, follows, post publishing, and feed aggregation.
- **Controllers**: `NewsFeedController` serves the UI; `NewsFeedApiController` exposes JSON endpoints.
...
├── src/main/java/com/randomproject/newsfeed/
│   ├── NewsFeedSystemPocApplication.java        # Boots the Spring application
│   ├── NewsFeedService.java                     # In-memory store + feed aggregation
│   ├── NewsFeedController.java                  # MVC controller for UI flows
│   ├── NewsFeedApiController.java               # REST endpoints for JSON access
...
## Configuration
- `server.port=8091` — avoid clashing with other POCs.
