...
export class Infrastructure {
  ...
  async connect() {
    const databaseUrl = process.env.ACCOUNT_DATABASE_URL || "memory://";
    const redisUrl = process.env.ACCOUNT_REDIS_URL || "memory://";
    const brokers = (process.env.ACCOUNT_KAFKA_BROKERS || "memory://").split(",");
    if (databaseUrl !== "memory://") {
      try {
        this.pool = new pg.Pool({ connectionString: databaseUrl });
        await this.pool.query("CREATE TABLE IF NOT EXISTS account_security_snapshots(id TEXT PRIMARY KEY, state JSONB NOT NULL)");
        await this.pool.query("CREATE TABLE IF NOT EXISTS account_security_events(event_key TEXT PRIMARY KEY, payload JSONB NOT NULL)");
    ...
    if (!brokers.includes("memory://")) {
      try {
        const kafka = new Kafka({ clientId: "personal-account-security-checkup", brokers });
        this.producer = kafka.producer();
        this.consumer = kafka.consumer({ groupId: "personal-account-security-checkup-poc" });
  ...
  async save(state: Record<string, unknown>) { this.state = structuredClone(state); if (this.pool) await this.pool.query("INSERT INTO account_security_snapshots VALUES('demo', $1::jsonb) ON CONFLICT(id) DO UPDATE SET state = EXCLUDED.state", [JSON.stringify(state)]); if (this.redis) await this.redis.set("account-security:snapshot", JSON.stringify(state)); }
  async load() { if (this.pool) { const result = await this.pool.query("SELECT state FROM account_security_snapshots WHERE id = 'demo'"); return result.rows[0]?.state || null; } return this.state; }
  async append(key: string, payload: Record<string, unknown>) { if (this.pool) await this.pool.query("INSERT INTO account_security_events VALUES($1, $2::jsonb) ON CONFLICT DO NOTHING", [key, JSON.stringify(payload)]); }
