...
export class Infrastructure {
  ...
  async connectPostgres(): Promise<void> {
    const databaseUrl = process.env.BACKUP_DATABASE_URL || "memory://";
    ...
    try {
      ...
      `);
      await this.pgPool.query(`
        CREATE TABLE IF NOT EXISTS backup_change_events (
      ...
      `);
      await this.pgPool.query(`
        CREATE TABLE IF NOT EXISTS backup_chunk_objects (
  ...
  async connectRedis(): Promise<void> {
    const redisUrl = process.env.BACKUP_REDIS_URL || "memory://";
    ...
    try {
      ...
      console.warn(`Redis unavailable (${(error as Error).message}); using memory queue state`);
      await this.closeRedis();
    }
  }

  async connectKafka(): Promise<void> {
    const brokers = (process.env.BACKUP_KAFKA_BROKERS || "memory://").split(",").filter(Boolean);
    if (brokers.includes("memory://")) return;
    try {
      const kafka = new Kafka({ clientId: "personal-backup-sync", brokers });
      this.producer = kafka.producer();
      this.consumer = kafka.consumer({ groupId: process.env.BACKUP_KAFKA_GROUP || "personal-backup-sync-poc" });
  ...
  async save(state: Record<string, unknown>): Promise<void> {
    ...
    if (this.pgPool) {
      await this.pgPool.query(
        `
        INSERT INTO backup_snapshots (id, state, updated_at)
    ...
    if (this.redis) await this.redis.setEx("backup:state", 300, JSON.stringify(state));
  }

  async load(): Promise<Record<string, unknown> | null> {
    if (this.pgPool) {
      const result = await this.pgPool.query("SELECT state FROM backup_snapshots WHERE id = $1", ["demo"]);
  ...
  async appendEvent(topic: string, eventKey: string, payload: Record<string, unknown>): Promise<void> {
    ...
    await this.pgPool.query(
      `
      INSERT INTO backup_change_events (event_key, topic, payload)
      VALUES ($1, $2, $3::jsonb)
  ...
  async mirrorJobs(jobs: Array<Record<string, unknown>>): Promise<void> {
    ...
    await this.redis.del("backup:jobs");
    for (const job of jobs) {
      if (job.status === "QUEUED" || job.status === "RETRY") {
        await this.redis.rPush("backup:jobs", JSON.stringify(job));
      }
    }
  }
