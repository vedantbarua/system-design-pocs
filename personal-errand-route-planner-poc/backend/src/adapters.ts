...
export class Infrastructure {
  ...
  async connect() {
    const databaseUrl = process.env.ERRAND_DATABASE_URL || "memory://";
    const redisUrl = process.env.ERRAND_REDIS_URL || "memory://";
    const brokers = (process.env.ERRAND_KAFKA_BROKERS || "memory://").split(",");
    if (databaseUrl !== "memory://") {
      try {
        this.pool = new pg.Pool({ connectionString: databaseUrl });
        await this.pool.query("CREATE TABLE IF NOT EXISTS errand_snapshots(id TEXT PRIMARY KEY, state JSONB NOT NULL)");
        await this.pool.query("CREATE TABLE IF NOT EXISTS errand_events(event_key TEXT PRIMARY KEY, payload JSONB NOT NULL)");
    ...
    if (!brokers.includes("memory://")) {
      try {
        const kafka = new Kafka({ clientId: "personal-errand-route", brokers });
        this.producer = kafka.producer();
        this.consumer = kafka.consumer({ groupId: "personal-errand-route-poc" });
  ...
  async save(state: Record<string, unknown>) { this.state = structuredClone(state); if (this.pool) await this.pool.query("INSERT INTO errand_snapshots VALUES('demo', $1::jsonb) ON CONFLICT(id) DO UPDATE SET state = EXCLUDED.state", [JSON.stringify(state)]); if (this.redis) await this.redis.set("errand:snapshot", JSON.stringify(state)); }
  async load() { if (this.pool) { const result = await this.pool.query("SELECT state FROM errand_snapshots WHERE id = 'demo'"); return result.rows[0]?.state || null; } return this.state; }
  async append(key: string, payload: Record<string, unknown>) { if (this.pool) await this.pool.query("INSERT INTO errand_events VALUES($1, $2::jsonb) ON CONFLICT DO NOTHING", [key, JSON.stringify(payload)]); }
