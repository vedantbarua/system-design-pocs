...
import { createSeededCoordinator, habitEventKey, type HabitEvent } from "./core.js";

const PORT = Number(process.env.PORT || 8260);
const HOST = process.env.HOST || "127.0.0.1";
const TOPIC = process.env.ROUTINE_EVENT_TOPIC || "personal.routine.events";
...
async function persist(event?: HabitEvent) { if (event) await infra.append(event.eventKey, event as unknown as Record<string, unknown>); await infra.save(coordinator.exportState()); }
...
app.post("/api/events", route(async (req, res) => {
  ...
  await infra.publish(TOPIC, input.habitId, input);
  const result = coordinator.ingest(input);
  await persist(result.event);
  res.status(result.duplicate ? 200 : 202).json(result);
}));
app.post("/api/events/publish", route(async (req, res) => {
  coordinator.habitById(req.body.habitId);
  habitEventKey(req.body);
  await infra.publish(TOPIC, req.body.habitId, req.body);
...
const server = app.listen(PORT, HOST, () => console.log(`Routine habit API listening on http://${HOST}:${PORT} (${infra.mode})`));
