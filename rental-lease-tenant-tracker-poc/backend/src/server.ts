...
import { createSeededRentalTracker, leaseEventKey, type LeaseEvent } from "./core.js";

const PORT = Number(process.env.PORT || 8310);
const HOST = process.env.HOST || "127.0.0.1";
const TOPIC = process.env.RENTAL_EVENT_TOPIC || "rental.lease.tenant.events";

const app = express();
let tracker = createSeededRentalTracker();
...
async function persist(event?: LeaseEvent) { if (event) await infra.append(event.eventKey, event as unknown as Record<string, unknown>); await infra.save(tracker.exportState()); }
...
app.post("/api/events", route(async (req, res) => {
  ...
  await infra.publish(TOPIC, input.recordId, input);
  const result = tracker.ingest(input);
  await persist(result.event);
  res.status(result.duplicate ? 200 : 202).json(result);
}));
app.post("/api/events/publish", route(async (req, res) => {
  tracker.recordById(req.body.recordId);
  leaseEventKey(req.body);
  await infra.publish(TOPIC, req.body.recordId, req.body);
...
app.post("/api/reset", route(async (_req, res) => { tracker = createSeededRentalTracker(); await persist(); res.json(tracker.snapshot()); }));

const server = app.listen(PORT, HOST, () => console.log(`Rental lease tenant tracker API listening on http://${HOST}:${PORT} (${infra.mode})`));
