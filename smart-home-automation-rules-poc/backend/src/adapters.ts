...
export class Infrastructure {
  ...
  async connectPostgres(): Promise<void> {
    const databaseUrl = process.env.AUTOMATION_DATABASE_URL || "memory://";
  ...
  async connectRedis(): Promise<void> {
    const redisUrl = process.env.AUTOMATION_REDIS_URL || "memory://";
    ...
    try {
      ...
      console.warn(`Redis unavailable (${(error as Error).message}); using memory state`);
      await this.closeRedis();
    }
  }

  async connectKafka(): Promise<void> {
    const brokers = (process.env.AUTOMATION_KAFKA_BROKERS || "memory://").split(",").filter(Boolean);
    if (brokers.includes("memory://")) return;
    try {
      const kafka = new Kafka({ clientId: "smart-home-automation", brokers });
      this.producer = kafka.producer();
      this.consumer = kafka.consumer({ groupId: process.env.AUTOMATION_KAFKA_GROUP || "smart-home-automation-poc" });
  ...
  async save(state: Record<string, unknown>): Promise<void> {
    ...
    if (this.pgPool) {
      await this.pgPool.query(
        `
        INSERT INTO automation_snapshots (id, state, updated_at)
    ...
    if (this.redis) await this.redis.setEx("automation:snapshot", 300, JSON.stringify(state));
  }

  async load(): Promise<Record<string, unknown> | null> {
    if (this.pgPool) {
      const result = await this.pgPool.query("SELECT state FROM automation_snapshots WHERE id = $1", ["demo"]);
  ...
  async appendEvent(topic: string, eventKey: string, payload: Record<string, unknown>): Promise<void> {
    ...
    await this.pgPool.query(
      `
      INSERT INTO automation_events (event_key, topic, payload)
      VALUES ($1, $2, $3::jsonb)
  ...
  async mirrorCommands(commands: Array<Record<string, unknown>>): Promise<void> {
    if (!this.redis) return;
    await this.redis.del("automation:commands");
    for (const command of commands) {
      if (command.status === "QUEUED" || command.status === "RETRY") {
        await this.redis.rPush("automation:commands", JSON.stringify(command));
      }
    }
  }
